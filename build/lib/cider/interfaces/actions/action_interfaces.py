...
class TreeActionInterface(ActionInterface):
    ...
    def __init__(self, configuration: ConfigurationWrapper, action: ActionInterface):
        self._configuration = configuration
        
        # Might as well configure the action
        self._action = action(configuration)


    def action(self, initial_nodes: Dict[str, List]):
        
        action_dict = {
            f"[green]{key}" : [self._action(k) for k in initial_nodes[key]] for key in initial_nodes['top_nodes']
        }
