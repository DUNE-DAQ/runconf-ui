...
- ``attributes`` - These are attributes of objects in OKS and require a bit more work to enable/disable. For example, the "TPG" attribute of a "ReadoutApplication" object
...
The YAML is organised into sub-groups such that the textual will produce tabs for each sub-group.
...
.. code-block:: yaml
    ...
    Settings:
        # List of classes to show on map panels
    ...
    PanelOptions:
        ...
        Panel:
            ...
            - SystemA: # A specific system
                ...
                # If we turn off all the subsystems do we also turn off the entire system
                ...
                components:
                    ...
                    # Component ID. If each_component_separate is true
                    ...
                    - id [str]:
                      ...
                      # Do these components also live in a separate subsystem?
                      separate_system [bool]: false
                      # Subsystem they live in [useful for things like TPC where you want to toggle individual CRPs]
                      system_label: None
        ...
        AttributeGroup: # Group of attributes to put in a tab
            - label [str]: # Internal label to keep textual sane
              ...
              - object_id [str, Optional]: # ID of object containing a given attribute. If left blank it will search for all objects of a given class
                ...
                tooltip [str, optional]: # Attribute to use as the tool tip i.e. "description" when highlighting box
