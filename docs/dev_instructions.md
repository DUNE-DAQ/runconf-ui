...
1. interfaces: These are interfaces with DAQ libraries
1. screens: Specific textual screens accessible to DAQ applications
1. utils: Internal utility functions
1. Widgets: Internal textual widgets
...
1. `ShifterInterfaceState` \[stored in `application_controller.py`\]. This object stores state information about the interface, this allows information to be passed between objects in a TUI implementation independent way.
1. `Actions`: This will be explained shortly in a second, but broadly `actions` take a configuration and perform an operation.
1. `Workflows`: More complicated actions, often requiring many steps
...
1. Defines an operation on the object
...
1. `QuitScreen`: PopUp screen shown when create and quit are pressed
1. `HelpScreen`: Help screen shown when help is pressed.
...
1. Tools for interacting with the DAQ configuration i.e. making configuration trees
1. Tools for handling files in general, for example saving and cleaning files
1. Tools for opening and generating objects from the shifter configuration YAMLs.
