...
class RepoManagerInterface(ABC, Generic[T]):
    ...
    def set_daq_version(self, version: T | None):
        if version not in self.get_available_daq_versions() and version is not None:
            raise DaqVersionException(f"{version} not in the available list of available DAQ versions ({self.get_available_daq_versions})")
    ...
    def get_runconf_ui_config_path(self):
        ui_config = self.conf_directory/"runconf-ui-settings"/f"{self.apparatus}.yml"
        if not ui_config.exists():
            raise MissingRunconfUIConfigException(f"{self.conf_directory} does not contain 'runconf-ui-settings/{self.apparatus}.yml'")
        return ui_config
