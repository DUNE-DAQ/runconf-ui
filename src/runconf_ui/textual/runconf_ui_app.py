...
class RunconfUIApp(App):
    ...
    def refresh_enabled_info(self):
        ...
        '''
        # Get the enable/disable information
        en_dis_info = self.backend.get_values()
        for panel in self.query(EnableDisableTabs):
            panel.update_panels(en_dis_info)
        
        # Get the rich trees
        en_dis_trees = self.backend.get_tree_views()
        for tree in self.query(RichTreeTabbed):
            tree.update_trees(en_dis_trees)
    ...
    def handle_version_selected(self, event: runconf_msg.DaqVersionSelectedMessage):
        ...
        availble_sessions = self.backend.get_sessions()
        
        for file_select in self.query(FileSelect):
            file_select.enable_session_select()
            file_select.update_sessions(availble_sessions)
...
if __name__=="__main__":
    ...
    # now we copy the runconf settings
    runconf_dir = tmp_path/"runconf-ui-settings"
    runconf_dir.mkdir(parents=True, exist_ok=True)
    
    shutil.copy(Path("/home/hwallace/scratch/dune_software/daq/daq_work_areas/NFD_DEV_260205_A9/pythoncode/runconf-ui-refactor/tests/test_files/dummy.yml"),
                runconf_dir/"dummy.yml"
                )
    
    context = RunconfContext(
        apparatus="dummy",
        conf_directory=tmp_path,
        use_local=True
    )
