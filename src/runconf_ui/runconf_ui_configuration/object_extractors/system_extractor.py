...
class SystemExtractor(MultiItemExtractor):
    ...
    def _build_system_structure(self) -> None:
        ...
        for system_name in self._system_names:
            if system_name not in self._tooltips:
                ...
                all_items = [
                    *self._get_components_for_system(system_name),
    ...
    def _subsystem_matches(
        self, subsystem: SubsystemExtractor, system_name: str
    ) -> bool:
    ...
    def _is_top_level_disabled(self, system_name: str | None) -> bool:
        ...
        return main_state in [
            SubsystemStatus.DISABLED,
            SubsystemStatus.TOP_LEVEL_DISABLED,
        ]
    ...
    def _calculate_system_state(self, system_name: str) -> SubsystemStatus:
        ...
        logging.debug(
            f"Mixed states found for {system_name}, returning PARTIALLY_ENABLED"
        )
    ...
    def _get_subsystem_state(self) -> SubsystemStatus:
        ...
        system_subsystems = [
            ...
            if subsystem.is_system
            and subsystem.get_state() != SubsystemStatus.STATE_NOT_DEFINED
        ]
    ...
    def _set_full_system_state(
        self, state: SubsystemStatus, system_name: str | None
    ) -> None:
        """
        ...
        other_system_states = [
            ...
            if (
                subsystem.is_system
                and subsystem.get_state() != SubsystemStatus.STATE_NOT_DEFINED
                and subsystem.system_name != system_name
            )
        ]
    ...
    def get_all_states(self) -> dict[str, SubsystemStatus]:
        ...
        for system_name in sorted(self._system_names):
            ...
            try:
                ...
                logging.debug(
                    f"Could not get state for {system_name} in {self.system_name}"
                )
    ...
    def find_components_with_wildcard(
        self, wildcard: str, class_name: str
    ) -> list[str]:
        """
        ...
        matching_components = []
        for dal in dals:
            component_id = ca.GetAttributeAction(
