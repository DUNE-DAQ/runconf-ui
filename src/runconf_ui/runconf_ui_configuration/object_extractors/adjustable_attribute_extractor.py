...
class AdjustableAttributeManager:
    def __init__(self, application_controller: ShifterInterfaceState, **kwargs):
        ...
        if object_id is None or "":
            ...
            self._object_list = [ca.GetDalObjectAction(
                self._application_controller.buffer_daq_config
        ...
        self._object_ids = [
            ca.GetAttributeAction(
                self._application_controller.buffer_daq_config
            )(obj, 'id') for obj in self._object_list]


        # Store initial values for the attribute
        self._init_values = {ca.GetAttributeAction(
            self._application_controller.buffer_daq_config
        )(obj, 'id'): ca.GetAttributeAction(
            self._application_controller.buffer_daq_config)(obj, self._attribute_name)
    ...
    def set_state(self, object_id: str, value: float) -> None:
        ...
        if self._upper_limit is not None and value > self._upper_limit:
            raise CiderOutOfBoundsException(f"Value {value} exceeds upper limit {self._upper_limit}.")
        
        if self._lower_limit is not None and value < self._lower_limit:
            raise CiderOutOfBoundsException(f"Value {value} is below lower limit {self._lower_limit}.")
    ...
    def get_state(self, object_id: str) -> float:
        for obj in self._object_list:
            if ca.GetAttributeAction(self._application_controller.buffer_daq_config)(obj, 'id') == object_id:
    ...
    def get_all_states(self) -> dict:
        return {obj: {'state': self.get_state(obj), 'attribute': self._attribute_name} for obj in self._object_ids}
    ...
    def get_init_values(self):
        ...
        return {
            obj_id: self._init_values[obj_id] for obj_id in self._object_ids
        }
