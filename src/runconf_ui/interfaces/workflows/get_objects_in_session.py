...
class GetSegmentAppsListAction(ActionInterface):
    def action(self, segment):
        ...
        for ss in ca.GetAttributeAction(self._daq_configuration)(segment, "segments"):
...
class GetObjectsInSessionAction(ActionInterface):
    ...
    def action(self, session_dal, applied_class: str, specific_objects=None):
        ...
        segment = ca.GetAttributeAction(self._daq_configuration)(session_dal, "segment")
        # Get all apps+segments in the segment
        full_app_list = GetSegmentAppsListAction(self._daq_configuration)(segment)
        ...
        for app in full_app_list:
            ...
            if (
                ...
                and ca.GetAttributeAction(self._daq_configuration)(app, "id")
                not in specific_objects
            ):
                continue
            # Check if the object is of the right class
            if ca.GetClassNameAction(self._daq_configuration)(app) != applied_class:
                continue
            apps.append(app)
        return apps
