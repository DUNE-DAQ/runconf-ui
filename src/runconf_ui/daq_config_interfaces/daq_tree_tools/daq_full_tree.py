...
class DaqFullTree(DaqConfTreeBase):
    def __init__(
        ...
        if self._class_filters and application_controller.buffer_daq_config is not None:
            ...
            for f in self._class_filters:
                try:
                    new_filters.update(
                        ca.GetDalsOfClassAction(
                            application_controller.buffer_daq_config
                        )(f)
                    )
                except Exception:
                    continue
    ...
    def build_tree(
        ...
        if not self.check_class_filter(dal_obj):
            ...
            if is_disabled:
                dal_branch = branch.add(
                    f"[bold red]{dal_name}[/bold red] [yellow]{dal_class}[/yellow] [dim] (disabled)[/dim]"
                )
            else:
                dal_branch = branch.add(
        ...
        for relationship in get_related_dals(dal_obj):
            ...
            for r in filtered_relationships:
                ...
                try:
                    ...
                    logging.warning(
                        f"Error checking if {r} is disabled, skipping", exc_info=True
                    )
                if dal_is_disabled:
                    self._disabled_objs.append(r)

                self.build_tree(
                    rel_branch, r, dal_is_disabled, session_name=session_name
                )
